...
export default function DailyPage() {
  // קריאת מספר היום מהכתובת (למשל ?day=1)
  ...
  useEffect(() => {
    const fetchDayContent = async () => {
      try {
        ...
        // שליפת הנתונים של היום הספציפי שנבחר
        ...
        setError("לא הצלחנו לטעון את היום המבוקש.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="max-w-4xl mx-auto p-4 space-y-6 dir-rtl font-sans min-h-screen flex flex-col">
      ...
      <header className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-800">יום {dayData.day}</h1>
            <p className="text-slate-500">{dayData.title}</p>
          </div>
        ...
        <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
          <div className="bg-indigo-600 h-full transition-all duration-500 ease-out" style={{ width: `${progressPercentage}%` }} />
        </div>

        <div className="flex gap-2 mt-6">
          <button onClick={() => setMode('study')} className={`flex-1 py-3 rounded-xl font-bold transition-all ${mode === 'study' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
            📚 מצב למידה
          </button>
          <button onClick={() => setMode('quiz')} className={`flex-1 py-3 rounded-xl font-bold transition-all ${mode === 'quiz' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>
            📝 מבחן ידע
          </button>
        </div>
      </header>
        ...
        {mode === 'study' ? (
          <div className="w-full max-w-xl">
            <div className="mb-6 transform transition-all duration-300 hover:scale-[1.01]">
              <WordCard key={currentWord.id} wordData={currentWord} onToggleKnown={toggleWordKnown} />
            </div>
            <div className="flex items-center justify-between gap-4 mt-4">
              <button onClick={handlePrev} disabled={currentIndex === 0} className="flex-1 flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-700 py-4 rounded-xl font-bold shadow-sm hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                <ChevronRight size={24} /> הקודם
              </button>
              <span className="text-lg font-bold text-slate-400">{currentIndex + 1} / {dayData.words.length}</span>
              <button onClick={handleNext} disabled={currentIndex === dayData.words.length - 1} className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white py-4 rounded-xl font-bold shadow-md hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
          ...
          <div className="w-full">
            <QuizMode words={dayData.words} onBackToStudy={() => setMode('study')} />
          </div>
        )}
      </main>
    </div>
